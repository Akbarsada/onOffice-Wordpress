<?php
	...
	class Project extends Estate {
		...
		//alle Objekte des Projekts
		public function  all_objects_of_project()	{
			$anzahl_objekte = get_post_meta( $this->id, 'anzahl_objekte', true );
			$hat_objekte = get_post_meta( $this->id, 'hat_objekte', true );
			$objects = array();
			
			if($hat_objekte == 1) {
				
				for ($i = 0; $i <= $anzahl_objekte-1; $i++) {
					
					$onOffice_id = get_post_meta($this->id, 'projekt_objekt_' . $i, true);
					...
					if ( $query->have_posts() ) {
						while ( $query->have_posts() ) {
							$query->the_post();
							$object = new Estate(get_the_ID());
							$objects[] = $object->estate_data();
						}
					} else {
						// no posts found
					}
		...
		public function  project_data()	{
			...
			*/
			...
			$object_data['_warmmiete'] =  get_post_meta($this->id, '_warmmiete', true);
			...
			$object_data['_barrierefrei'] =  get_post_meta($this->id, '_barrierefrei', true);
			$object_data['_anzahl_objekte'] =  get_post_meta($this->id, 'anzahl_objekte', true);
		...
		public static function getAll_projects() {
			$project_array  = array();
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					$project = new Project(get_the_ID());
					$project_array[] = $project->project_data();
				}
			}

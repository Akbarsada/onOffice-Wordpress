    ...
    if(
        ...
        if (in_array($document['elements']['documentAttribute'], $documentAttributeArray)) {
            ...
            if(!empty($fileResponse['data']['records'][0]['elements']['content'])){
                ...
                mkdir(EUGEN_ONOFFICE_PLUGIN_DIR.'files/projects/documents/'.$project['id'], 0777, true);
                $docdir = EUGEN_ONOFFICE_PLUGIN_DIR.'files/projects/documents/'.$project['id'];
                file_put_contents($docdir.'/'.$document['id']. '.pdf', $pdf);
                $doc_array1['type']=$document['elements']['type'];
                $doc_array1['url']= EUGEN_ONOFFICE_PLUGIN_URL.'files/projects/documents/'.$project['id'].'/'.$document['id']. '.pdf';
